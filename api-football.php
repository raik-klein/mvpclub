...
function mvpclub_ajax_search_players(){
    ...
    $res = mvpclub_api_football_search_players($query, $season, $league_id);
    if(is_wp_error($res)){
...
function mvpclub_render_api_football_settings_page() {
    ...
    if (isset($_GET['add_player']) && isset($_GET['_wpnonce']) && wp_verify_nonce($_GET['_wpnonce'], 'mvpclub_add_player')) {
        ...
        $season = isset($_GET['player_season']) ? intval($_GET['player_season']) : null;
        $import_result = mvpclub_import_player_post($pid, $season);
    ...
    $search_season = isset($_GET['player_season']) ? intval($_GET['player_season']) : '';
    $results = array();
    if ($player_search !== '') {
        $league_id = '';
        if ($search_league !== '') {
        ...
        $result = mvpclub_api_football_search_players($player_search, $search_season ?: null, $league_id);
    ...
    <div class="wrap">
        ...
        <form id="mvpclub-player-search-form" method="get">
            ...
            <select name="player_season">
                <option value="">Saison</option>
                <?php for($y=2020;$y<=2025;$y++): ?>
                    <option value="<?php echo $y; ?>" <?php selected($search_season, $y); ?>><?php echo $y; ?></option>
                <?php endfor; ?>
            </select>
        ...
        <table id="mvpclub-search-results" class="widefat fixed striped">
            ...
            <tbody>
                <?php if (!empty($results)) : foreach ($results as $row) :
                    ...
                    $link = wp_nonce_url(admin_url('admin.php?page=mvpclub-api-football&add_player=' . $p['id'] . '&player_search=' . urlencode($player_search) . '&player_season=' . intval($search_season) . '&search_league=' . urlencode($search_league)), 'mvpclub_add_player');
                ?>
                    <tr>
